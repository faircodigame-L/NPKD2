...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct 10 13:40:16 2025
...
TensorFlow 2 / Keras script that:
- Loads CIFAR-10, resizes to VGG16 input size (224x224)
- Builds a custom model cifar10_CNN
- Implements FGSM, PGD, and Carlini-Wagner (L2) attacks
- Evaluates model on clean and adversarial examples
- 

Notes:
- Requires TensorFlow 2.x
- CW implementation below is a straightforward L2-optimized version inspired by Carlini & Wagner (2017).
  It is slower than library implementations but self-contained.
...
model.add(Conv2D(132,(3,3), activation='relu', kernel_initializer='he_uniform'))
...
# fit the model
history = model.fit(x_train, y_train, epochs=15, batch_size=132, validation_data=(x_test,y_test),verbose=2)
...
print('CNN clean Model Accuracy on CIFAR10 : %.4f' % (acc*100))
# make a prediction
image = x_train[1]
...
# CW-L2: optimization-based L2 attack (untargeted)
# -----------------------------
...
print('NN Model Accuracy on CIFAR10 after Attack: %.3f' % (pois_acc*100))
...
cifar10_conf = confusion_matrix(y_test, np.argmax(test_predictions,axis=1))

cifar10_conf

plt.imshow(cifar10_conf, interpolation='nearest', cmap=plt.cm.Blues)
plt.title('CIFAR10 CNN Confusion matrix')
plt.colorbar()
...
plt.imshow(np.resize(fgm[7],(32,32)), cmap=plt.get_cmap('gray'))

plt.imshow(fgm[7])
plt.imshow(x[7])

plt.imshow(np.resize(x[7],(32,32)), cmap=plt.get_cmap('gray'))


for i in range(20):
  plt.subplot(4, 5, i+1)
  plt.imshow(np.resize(fgm[i],(32,32)), cmap=plt.get_cmap('gray'))
plt.show()
...
print("RBF deviations obtained between clean and FGSM CIFAR10 \n ", Rbf_d_fgm)
print("RBF mean Deviation fgsm poisoning on CIFAR10 : ", Rbf_d_fgm.mean())


plt.title ='RBF measure between clean and fgsm CIFAR10 images'
plt.hist(Rbf_d_fgm, label='RBF measure between clean and fgsm CIFAR10 images')
plt.show()

Rbf_d_clean = rbf_deviation(c_s, c_s,None)
print("RBF mean Deviation clean CIFAR10 : ", Rbf_d_clean.mean())


L_d_fgm = Laplace_deviation(c_s, p_s_fgm,1)
print("Laplace Deviation fgsm poisoning on CIFAR10: ", L_d_fgm.mean())
...
plt.title = "RBF Deviation: Clean vs FGSM-Poisoned CIFAR10"
...
plt.title = "Laplace Deviation: Clean vs FGSM-Poisoned CIFAR10"
...
print('CNN Model Accuracy on CIFAR10 after Attack: %.2f' % (pois_acc*100))
print('No.of Poisoning points : ', n_pois)
...
pgd_p_s =  np.resize(pois_pgd_samp, (32, 32))
pgd_p_s.ndim
...
plt.title = "RBF Deviation: Clean vs PGD-Poisoned CIFAR10"
plt.xlabel("RBF Deviation (1 - similarity)")
plt.ylabel("Frequency")
plt.legend()
plt.show()
...
plt.title = "Laplace Deviation: Clean vs PGD-Poisoned CIFAR10"
plt.xlabel("Laplace Deviation (1 - similarity)")
plt.ylabel("Frequency")
plt.legend()
plt.show()
...
print('CNN Model Accuracy on CIFAR10 after Attack: %.3f' % (pois_acc*100))
print('No.of Poisoning points : ', n_pois)
...
# define subplot
  plt.subplot(5, 5, i+1)
	# plot raw pixel data
  #plt.imshow(np.resize(cw_adv[i],(32,32)), cmap=plt.get_cmap('gray'))
  plt.imshow(cw_adv[i])
# show the figure
ax.set_title("CW poisoned samples")
plt.tight_layout()
plt.show()
...
cw_p_s =  np.resize(cw_adv, (32, 32))
cw_p_s.ndim

# CW perturbation Deviation

Rbf_d_cw = rbf_deviation(c_s, cw_p_s, 1)
#print("RBF deviations for CW poisoned CIFAR10: ", Rbf_d_cw)
...
#print("Laplacian deviations for CW poisoned CIFAR10:", L_d_cw)
...
plt.title = "RBF Deviation: Clean vs CW-Poisoned CIFAR10"
plt.xlabel("RBF Deviation (1 - similarity)")
plt.ylabel("Frequency")
plt.legend()
plt.show()
...
plt.title = "Laplace Deviation: Clean vs CW-Poisoned CIFAR10"
plt.xlabel("Laplace Deviation (1 - similarity)")
plt.ylabel("Frequency")
plt.legend()
plt.show()
...
p_perm_rbf_clean = permutation_test(rbf_clean, rbf_clean)
...
print("Permutation p-value (RBF clean):", p_perm_rbf_clean)
...
p_perm_lap_clean = permutation_test(lap_clean, lap_clean)
...
print("Permutation p-value (Laplace clean):", p_perm_lap_clean)
